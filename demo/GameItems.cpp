...
namespace Demo
{
	...
	void ItemData::LoadData()
	{
		...
		itemRegistry[4] = ConsumableItem(4, L"Buff 10 Defense & Heal 12HP",
			...
			{
				CombatModifier{ ModifierType::BuffDefense, 4, 10.0f, true },
				CombatModifier{ ModifierType::HealHP, 0, 12.0f, true }
			},
			{ 96, 0, 119, 35 });

		itemRegistry[5] = ConsumableItem(5, L"Mega Heal Elixir",
			L"Instant Heal 45HP",
			{ CombatModifier{ ModifierType::HealHP, 0, 45.0f, true } },
			{ 0, 36, 23, 71 });

		itemRegistry[6] = ConsumableItem(6, L"Iron Wall Shield",
			L"Buff 40.0 Defense for 1 turn",
			{ CombatModifier{ ModifierType::BuffDefense, 1, 40.0f, true } },
			{ 24, 36, 47, 71 });

		itemRegistry[7] = ConsumableItem(7, L"Berserker's Wrath",
			L"Buff 35 Damage for 1 turn",
			{ CombatModifier{ ModifierType::BuffDamage, 1, 35.0f, true } },
			{ 48, 36, 71, 71 });

		itemRegistry[8] = ConsumableItem(8, L"Paladin's Blessing",
			L"Instant Heal 10HP & Buff 12 Defense and Damage for 2 turns",
			{
				CombatModifier{ ModifierType::HealHP, 0, 10.0f, true },
				CombatModifier{ ModifierType::BuffDefense, 2, 12.0f, true },
				CombatModifier{ ModifierType::BuffDamage, 2, 12.0f, true }
			},
			{ 72, 36, 95, 71 });
		...
		itemRegistry[9] = ConsumableItem(9, L"Titan's Resolve",
			...
			{
				CombatModifier{ ModifierType::BuffDefense, 3, 25.0f, true },
				CombatModifier{ ModifierType::BuffDamage, 3, 5.0f, true }
			},
			{ 96, 36, 119, 71 });
